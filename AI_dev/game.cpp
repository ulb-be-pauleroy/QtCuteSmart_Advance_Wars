...
*/
		...
		Terrain* t = new Terrain(5,3,1);
		this->map[5][3].push_back(t);
...
{
	this->lock->lockForRead(); //otherwise causes empty tiles segfaults, doesnt seem to work
	int mov =1;
	vector<GameObject*>& tile = this->map[x][y];
	//vector<GameObject*> tile = vector<GameObject*>(this->map[x][y]); // try instead of lock, bad idea
	for(unsigned int i=0;i<tile.size();i++){
